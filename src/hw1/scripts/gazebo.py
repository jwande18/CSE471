...
def run_gazebo_simulation(line_no, line):
    ...
    dimension = int(line[0].strip())
    obstacles = int(line[1].strip())
    seed = int(line[2].strip())
    
    actions = line[-1].strip()
    
    # Generate the maze.
    generate_maze(dimension, obstacles, seed)
    
    # Execute it on Gazebo.
    print("[BEGIN] Running Gazebo simulation on line_no=%u with dimension=%u"
        ", obstacles=%u, seed=%u, plan=%s" % (line_no, dimension, obstacles,
    ...
    print("[  END] Running Gazebo simulation on line_no=%u with dimension=%u"
        ", obstacles=%u, seed=%u, plan=%s" % (line_no, dimension, obstacles,
        seed, str(actions.split("_"))))
